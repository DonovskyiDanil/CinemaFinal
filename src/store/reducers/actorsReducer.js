...
export default function actorsReducer(state = initialState, { type, payload }) {
  switch (type) {
    ...
    case ACTIONS_TYPES.POST_ACTOR_SUCCESS:
      return { ...state, actors: [...state.actors, payload], isFetching: false };
    case ACTIONS_TYPES.PUT_ACTOR_SUCCESS:
      return { ...state, actors: state.actors.map(actor => actor.id === payload.id ? payload : actor), isFetching: false };
    case ACTIONS_TYPES.DELETE_ACTOR_SUCCESS:
      return { ...state, actors: state.actors.filter(actor => actor.id !== payload), isFetching: false };
